...
def _parse_client_hello_body(data: bytes) -> dict:
    ...
    for i in range(0, cipher_suites_len, 2):
        cipher_suites.append(int.from_bytes(data[pos + i:pos + i + 2], "big"))
            ...
            if ext_type == 0x0000:
                ...
                if ext_data:
                    ...
                    for i in range(0, n, 2):
                        supported_versions.append(int.from_bytes(ext_data[1 + i:1 + i + 2], "big"))
            elif ext_type == 0x000d:
                if len(ext_data) >= 2:
                    n = int.from_bytes(ext_data[0:2], "big")
                    for i in range(0, n, 2):
                        sig_algs.append(int.from_bytes(ext_data[2 + i:2 + i + 2], "big"))
            elif ext_type == 0x0010:
                if len(ext_data) >= 2:
                    list_len = int.from_bytes(ext_data[0:2], "big")
                    p = 2
                    end_alpn = 2 + list_len
                    while p < end_alpn and p < len(ext_data):
                        proto_len = ext_data[p]
                        proto = ext_data[p + 1:p + 1 + proto_len]
...
def _extract_client_hello_bytes(raw: bytes) -> bytes:
...
def ja4_fingerprint(clienthello) -> str:
    """
    Compute the JA4 fingerprint for a single TLS ClientHello.
    `clienthello` is a hex string of the raw ClientHello bytes.
    """
    if isinstance(clienthello, str):
        raw = bytes.fromhex(clienthello.strip())
    else:
        raw = clienthello

    body = _extract_client_hello_bytes(raw)
    parsed = _parse_client_hello_body(body)

    protocol = "t"

    version_candidates = [v for v in parsed["supported_versions"] if v not in GREASE_VALUES]
    version_val = max(version_candidates) if version_candidates else parsed["client_version"]
    version = TLS_VERSION_MAP.get(version_val, "00")

    sni = "d" if parsed["sni_present"] else "i"

    ciphers = [c for c in parsed["cipher_suites"] if c not in GREASE_VALUES]
    cipher_count = min(len(ciphers), 99)

    ext_types_all = [t for t, _ in parsed["extensions"] if t not in GREASE_VALUES]
    ext_count = min(len(ext_types_all), 99)

    alpn_part = "00"
    if parsed["alpn_protocols"]:
        first_alpn = parsed["alpn_protocols"][0]
        if len(first_alpn) > 0:
            try:
                s = first_alpn.decode("ascii")
                if s.isalnum():
                    alpn_part = f"{s[0]}{s[-1]}"
                else:
                    raise ValueError
            except (UnicodeDecodeError, ValueError, IndexError):
                hex_s = first_alpn.hex()
                if hex_s:
                    alpn_part = f"{hex_s[0]}{hex_s[-1]}"

    ja4_a = f"{protocol}{version}{sni}{cipher_count:02d}{ext_count:02d}{alpn_part}"

    if ciphers:
        ja4_b = hashlib.sha256(",".join(sorted(f"{c:04x}" for c in ciphers)).encode()).hexdigest()[:12]
    else:
        ja4_b = "000000000000"

    ext_for_hash = sorted(f"{t:04x}" for t in ext_types_all if t not in (0x0000, 0x0010))
    sig_alg_hex = [f"{s:04x}" for s in parsed["sig_algs"]]

    if not ext_for_hash and not sig_alg_hex:
        ja4_c = "000000000000"
    else:
        ext_str = ",".join(ext_for_hash)
        ja4_c_input = f"{ext_str}_{','.join(sig_alg_hex)}" if sig_alg_hex else ext_str
